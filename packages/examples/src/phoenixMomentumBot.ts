...
export class PhoenixMomentumBot {
	...
	private eventEmitter: StrictEventEmitter<EventEmitter, PhoenixAccountEvents> =
		new EventEmitter();
	 ...
	 * @param market - address of Phoenix market (i.e. SOL/USDC)
	 ...
	 * @param marketCacheSize - Number of market price snapshots to cache
	 * @param tf - Timeframe to use for trading
	 * @param simulate - Simulate transactions instead of sending
	 */
	...
	async initialize(): Promise<void> {
		...
		const markets = Array.from(this.phoenixClient.marketStates.keys()).map(
			(k) => {
				return {
	...
	async placeLimitOrder(params: {
		...
		const ixs = [];
		if (cancelOrders) {
			ixs.push(await this.client.cancelOrdersIx(this.fundKey, marketState));
		}
		ixs.push(
